...
## Files / Locations
- Flows: [orchestration/flows](orchestration/flows)
	- `01_hello_world.yml`
	- `02_python.yml` (contains a Python Script task used as an example)

## Notes & Troubleshooting
- The dataset for 2021 is partial (Jan → Jul). Do not attempt to backfill beyond 2021-07.
- If Kestra run tasks fetch files from GitHub releases, ensure network access to `github.com`.
- For large native builds (e.g., installing `cmake` locally), prefer prebuilt binaries (Homebrew bottles) to avoid long compile times.

## Contact
If you want me to: run tests, add CI, or commit these changes to a branch and push, tell me which action to take next.

---
Generated: Module 2 — Kestra Backfill & BigQuery Validation
